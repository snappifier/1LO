...
const LinkComponent = ({linkArray}) => {
    ...
    return (<div className="w-full flex flex-col">
        {linkArray.map((link) => (
            <Link
                to={link["Plik"]?.url ? getStrapiMedia(link["Plik"].url) : "#"}
                target="_blank"
...
const TemplateSameLinki = ({api}) => {
    const { data } = useSuspenseQuery({
        queryKey: ["page", api, "populate=Sekcja+nested"],
        queryFn: () =>
            get(api, {
                params: {
                    "populate[Sekcja][populate][Paragraf][populate]": "*",
                    "populate[Sekcja][populate][Linki][populate]": "*",
                    "populate[Sekcja][populate]": "Media",
