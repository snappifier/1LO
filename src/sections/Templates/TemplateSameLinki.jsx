...
const TemplateSameLinki = ({api, type}) => {
    const { data } = useSuspenseQuery({
        queryKey: ["page", api],
        queryFn: () => get("program-wychowawczo-profilaktyczny", {
            params: { "populate[Linki][populate]": "*" }  // tylko to, bez populate="*"
        }),
    });
            ...
            <div className="w-full pt-36 md:pt-40 pb-16 md:pb-20 flex flex-col items-center  min-h-[80vh]">
                <div className="w-[92%] sm:w-[90%] lg:w-[80%]">
                    <div className="w-full flex flex-col mb-4 sm:mb-6 text-wrap">
                        <p className="w-full text-3xl sm:text-4xl lg:text-5xl font-extralight">
                            {page["Naglowek"]}
                        </p>
                    </div>
                    <div className="w-max h-max flex flex-col gap-4 sm:gap-6">
                        {page["Linki"].map((item) => {
                            ...
                            return <Link to={item["Media"][0]?.url ? getStrapiMedia(item["Media"][0].url) : null} target={"_blank"} className="w-max h-max">
                                <motion.div
                                    className="w-full cursor-pointer flex items-center justify-between gap-3 text-wrap"
                                    ...
                                    <p className="text-xl mt-1 font-[poppins] font-light text-slate-700">{item["NazwaLinku"]}</p>
                                </motion.div>
                            </Link>
                        })}
